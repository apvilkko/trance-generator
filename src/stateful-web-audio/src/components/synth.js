import { connect, getContext, ensurePositive } from "../util";
...
export const playSynth = ({ context, pitch = 0, instance, delta = 0 }) => {
  instance.vcos.forEach(vco => {
    const freq = offsetFreq(BASE_FREQ, pitch, vco.params.detune);
    vco.instance.frequency.setValueAtTime(
      freq,
      ensurePositive(context.currentTime + delta)
    );
  });
};

export const setSynthParam = (ctx, instance, params, value, delta = 0) => {
  const time = ensurePositive(getContext(ctx).currentTime + delta);
